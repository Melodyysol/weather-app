...
export function pageHead() { 
  const pageHeadHTML = `
    <div class="flex flex-wrap justify-between content-center">
      ...
      <div class="js-units-display relative cursor-pointer bg-[#3d3b5e] flex justify-center gap-x-2 w-30 content-center py-2 rounded-md">
        <img src="weather-app-main/assets/images/icon-units.svg" alt="Setting Icon">
        <span class="text-[0.75rem]">Units</span>
        <img class="w-4" src="weather-app-main/assets/images/icon-dropdown.svg" alt="Dropdown Icon">
        <div class="js-units-conversion absolute flex-col gap-y-5 bg-[#3d3b5e] w-60 mt-2 rounded-xl px-2 pt-4 pb-10 z-20 right-0 top-full overflow-auto border border-gray-600 hidden">
    ...
    <div class="js-search-cont mt-10 flex flex-col gap-y-3 md:flex-row md:gap-x-4 md:m-auto md:w-3/4 lg:w-1/2 md:mt-12">
      <div class="js-search-container z-10 relative bg-[#3d3b5e] flex rounded-xl p-3 gap-x-4 content-center hover:opacity-90 md:flex-1">
        ...
        <div class="js-search-suggestion absolute bg-[#3d3b5e] w-full mt-1 rounded-md p-2 z-10 left-0 top-full max-h-60 overflow-y-auto border border-gray-600 suggestion-hidden">
        </div>
      </div>
